...
The header orders under `orders/` were measured on 2026-09-08 against a local HTTP/1.1 server that keeps headers in arrival order, from Chrome, Edge and Brave 152 on Windows, each started headless, for a navigation and for an image the page fetched. The three browsers sent the same order; Brave adds `sec-gpc` after `accept`. The two request kinds differ, which is why there are two files.
...
- No built-in header order. What exists is two files measured on one major, one platform, one transport and headless builds. That is not enough to ship as a table that fails other people's requests, so the check takes a file and the files say what they cover. Over HTTP/2 the pseudo-headers are not sent, `priority` is, and a proxy or a HAR export may not keep the order the browser used. Hold a request against an order only when the order it shows is the order that reached the wire.
